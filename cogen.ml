...
let rec eval = function
  ...
  | BinOp(p, op, e1, e2) ->
     ...
     .< match op, .~(v1 env), .~(v2 env) with
      ...
      | _ -> err "Can't happen (non-integer argument to binop)">.
  | IfExp(p, e1, e2, e3) ->
     let test = eval e1 in
     ...
     .< match .~(test env) with
      ...
      | _ -> err "Can't happen (nonbool condition)" >.
  | FunExp (_, id, _, e) ->
     let body = eval e in
  ...
  | AppExp (p, e1, e2) ->
     ...
     .< match .~(proc env) with
          Fun f -> f .~(arg env)
        | _ -> err "Can't happen (application of nonprocedure value)">.
  | TSFunExp (_, id, e) ->
  ...
  | TAppExp (p, e, _) ->
     ...
     .< match .~(tfun env) with
          TFun f -> f ()
        | _ -> err "Can't happen (application of non-tyabs)" >.
  | CastExp (p, e, ty1, ty2) ->
     let v = eval e in
